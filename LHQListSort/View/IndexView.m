...
#define kIndexViewInsetTop (self.translucentForTableViewInNavigationBar ? UIApplication.sharedApplication.statusBarFrame.size.height + 44 : 0)
...
@property (nonatomic, strong) NSMutableArray<TextLayer *> *subTextLayers;
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < self.subTextLayers.count; i++) {
        TextLayer *textLayer = self.subTextLayers[i];
...
- (void)configSubLayersAndSubviews
{
    ...
    if (countDifference > 0) {
        ...
        for (int i = 0; i < -countDifference; i++) {
            TextLayer *textLayer = self.subTextLayers.lastObject;
    ...
    if (self.subTextLayers.count == 0) {
        ...
        self.currentSection = self.searchLayer ? IndexViewSearchSection : 0;
    } else {
...
{
    NSInteger currentSection = IndexViewInvalidSection;
    if (self.delegate && [self.delegate respondsToSelector:@selector(sectionOfIndexView:tableViewDidScroll:)]) {
        ...
        if ((currentSection >= 0 && currentSection != IndexViewInvalidSection)
            || currentSection == IndexViewSearchSection) {
    ...
    CGFloat insetTop = kIndexViewInsetTop;
...
{
    if ((self.currentSection < 0 && self.currentSection != IndexViewSearchSection)
        || self.currentSection >= (NSInteger)self.subTextLayers.count) {
        return;
    }
    
    CGFloat insetTop = kIndexViewInsetTop;
    if (self.currentSection == IndexViewSearchSection) {
...
- (void)showIndicator:(BOOL)animated
{
    ...
    TextLayer *textLayer = self.subTextLayers[self.currentSection];
    if (self.configuration.indexViewStyle == IndexViewStyleDefault) {
...
- (void)setCurrentSection:(NSInteger)currentSection
{
    ...
    if ((currentSection < 0 && currentSection != IndexViewSearchSection)
